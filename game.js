...
const translations = {
  ...
  zh: {
    ...
    intro: "在北欧港城风格的卡通像素街区跳跃、收集能量球和宝石，避开海鸥、鸽子与陷阱，抵达中央车站门口通关。",
...
function applyLanguage(lang){
  ...
  ui.languageButton.textContent = translate("langButton");
  if (player) updateHud();
}
...
let selectedHero = "blue";
let running = false;
let cameraX = 0;
let lastTime = 0;
let wealthGems = Number(localStorage.getItem("lumiGems") || 0);

const level = {
  platforms: [
    {x:0,y:475,w:520,h:65},
    {x:610,y:420,w:220,h:30},
    {x:910,y:360,w:230,h:30},
    {x:1220,y:455,w:500,h:65},
    {x:1820,y:400,w:230,h:30},
    {x:2140,y:335,w:220,h:30},
    {x:2440,y:455,w:450,h:65},
    {x:3020,y:475,w:480,h:65}
  ],
  traps: [
    {x:540,y:505,w:70,h:35,type:"pit"},
    {x:1720,y:492,w:90,h:28,type:"water"},
    {x:2350,y:493,w:65,h:28,type:"crack"},
    {x:2720,y:418,w:96,h:36,type:"car"},
    {x:2920,y:438,w:130,h:32,type:"train"}
  ],
...
function resetGame() {
  ...
  enemies = [
    {x:720,y:386,w:38,h:28,type:"pigeon",vx:-0.7,home:720,range:120},
    {x:1420,y:420,w:42,h:32,type:"seagull",vx:1.2,home:1420,range:260},
    {x:2210,y:280,w:42,h:32,type:"seagull",vx:-1.4,home:2210,range:180}
  ];
...
function collectGlow(x,y,color){
  for(let i=0;i<14;i++){
    particles.push({
      x,
      y,
...
function activatePower(){
  if(player.energy >= 10 && !player.invincible){
    player.energy = 0;
    player.invincible = true;
    player.powerTimer = 5;
    collectGlow(player.x+20, player.y+25, "#ffe66d");
    updateHud();
  }
}
...
function update(dt){
  ...
  player.crouch = keys.ArrowDown || keys.KeyS;
  player.vx = move * (player.crouch ? 2 : 4.2);
  ...
  for(const o of orbs){
    if(!o.taken && Math.hypot(player.x+20-o.x, player.y+28-o.y) < 34){
      o.taken = true;
      player.score += 50;
      player.energy++;
      collectGlow(o.x,o.y,"#ffd84d");
      if(player.energy >= 10) player.energy = 10;
      updateHud();
    }
  }

  for(const g of gems){
    if(!g.taken && rects(player,g)){
      g.taken = true;
      player.gems++;
      player.score += 100;
      localStorage.setItem("lumiGems", player.gems);
      collectGlow(g.x,g.y,"#9ffcff");
      updateHud();
    }
  }
  ...
  for(const e of enemies){
    ...
    if(Math.abs(e.x-player.x) < 180 && e.type === "pigeon") e.vx = Math.sign(player.x-e.x) * 1.8;
    if(rects(player,e)) player.invincible ? smash(e) : hurt(false);
  }

  for(const trap of level.traps){
    if(!trap.smashed && rects(player,trap)){
      if(player.invincible && (trap.type === "car" || trap.type === "train")){
        trap.smashed = true;
        collectGlow(trap.x+trap.w/2, trap.y, "#ccc");
      } else {
        hurt(trap.type === "pit" || trap.type === "water");
      }
    }
  }
  ...
  if(player.invincible){
    ...
    if(player.powerTimer <= 0) player.invincible = false;
  }
  ...
  cameraX = Math.max(0, Math.min(worldWidth-W, player.x - W*.42));
  updateHud();
}
...
function smash(e){
  e.x = -9999;
  player.score += 120;
  collectGlow(player.x,player.y,"#ffe66d");
  updateHud();
}

function hurt(fall){
  if(player.hitTimer > 0 || player.invincible) return;

  player.life--;
  player.hitTimer = 1.1;

  if(fall){
    player.x = Math.max(80, cameraX+70);
    player.y = 250;
    player.vy = 0;
  }
...
function endGame(win){
  running = false;
  if(win) localStorage.setItem("lumiGems", player.gems);

  ui.messageTitle.textContent = win ? translate("winTitle") : translate("loseTitle");
  ui.messageText.textContent = win ? translate("winText")(player.gems, player.score) : translate("loseText");
...
function updateHud(){
  ...
  ui.gem.textContent = player.gems;
  ui.energy.textContent = `${player.energy}/10`;

  ui.power.textContent = player.invincible
    ? `${translate("active")} ${player.powerTimer.toFixed(1)}s`
    : (player.energy >= 10 ? translate("ready") : translate("notReady"));

  ui.power.classList.toggle("ready", player.energy >= 10 || player.invincible);
}
...
function draw(){
  ...
  ctx.beginPath();
  ctx.arc(760-cameraX*.08, 90+Math.sin(time)*14, 34, 0, 7);
  ctx.fill();
  ...
  for(const o of orbs){
    if(!o.taken){
      ...
      ctx.beginPath();
      ctx.arc(o.x,o.y,o.r+Math.sin(time*6)*2,0,7);
      ctx.fill();
  ...
  for(const g of gems){
    if(!g.taken){
      ...
      ctx.lineTo(g.x+18,g.y+9);
      ctx.lineTo(g.x+9,g.y+18);
...
function drawScenery(){
  ctx.save();
  ctx.translate(-cameraX*.35,0);

  for(let x=-100;x<worldWidth;x+=620){
    ctx.fillStyle = "#cf4b54";
    ctx.fillRect(x+60,260,140,160);

    ctx.fillStyle = "#7b2d3b";
    ctx.beginPath();
    ctx.moveTo(x+40,260);
    ctx.lineTo(x+130,185);
    ctx.lineTo(x+220,260);
    ctx.fill();

    ctx.fillStyle = "#f5d49a";
    ctx.fillRect(x+360,300,160,110);

    ctx.fillStyle = "#34506f";
    ctx.fillRect(x+382,325,32,48);
    ctx.fillRect(x+450,325,32,48);

    ctx.fillStyle = "#2f4467";
    ctx.fillRect(x+520,365,150,22);

    ctx.fillStyle = "#fff";
    ctx.fillText("LUMINKI",x+530,360);
  }
...
function drawExit(exit){
  ctx.fillStyle = "#2e436b";
  ctx.fillRect(exit.x,exit.y,exit.w,exit.h);

  ctx.fillStyle = "#f0c36a";
  ctx.fillRect(exit.x+18,exit.y+28,exit.w-36,exit.h-28);

  ctx.fillStyle = "#22314f";
  ctx.fillRect(exit.x+45,exit.y+70,60,90);
}

function drawTrap(trap){
  if(trap.smashed){
    ctx.fillStyle = "rgba(170,170,170,.35)";
    ctx.fillRect(trap.x,trap.y,trap.w,trap.h);
    return;
  }

  ctx.fillStyle =
    trap.type === "water" ? "#2fb8ff" :
    trap.type === "car" ? "#e94b4b" :
    trap.type === "train" ? "#58677a" :
    "#1b2035";
...
function drawEnemy(enemy){
  ctx.fillStyle = enemy.type === "seagull" ? "#f4f7fb" : "#9aa0aa";
  ctx.fillRect(enemy.x,enemy.y,enemy.w,enemy.h);

  ctx.fillStyle = "#333";
  ctx.fillRect(enemy.x+26,enemy.y+8,4,4);

  ctx.fillStyle = "#ffb23c";
  ctx.fillRect(enemy.x+enemy.w-2,enemy.y+12,10,5);
}

function drawPlayer(time){
  const p = player;
  const blue = selectedHero === "blue";

  if(p.invincible){
    ctx.strokeStyle = "#ffe66d";
    ctx.lineWidth = 5;
    ctx.beginPath();
    ctx.arc(p.x+20,p.y+30,44+Math.sin(time*8)*4,0,7);
    ctx.stroke();

    ctx.strokeStyle = `hsl(${time*160%360},100%,70%)`;
    ctx.stroke();
  }

  ctx.fillStyle = blue ? "#2478e6" : "#ff62b3";
  ctx.fillRect(p.x,p.y+16,p.w,p.h-16);

  ctx.fillStyle = blue ? "#bfeeff" : "#ffe0ef";
  ctx.fillRect(p.x+7,p.y,p.w-14,24);

  ctx.fillStyle = "#1d2440";
  ctx.fillRect(p.x+(p.face>0?24:10),p.y+8,5,5);

  if(p.crouch) ctx.fillRect(p.x+5,p.y+44,p.w-10,14);
  if(p.spawnTimer>0) ctx.fillRect(p.x-8,p.y-18,54,8);

  if(!p.vx && p.onGround && Math.sin(p.idleTimer*2)>0.85){
    ctx.fillStyle = "#fff";
    ctx.fillText(blue ? translate("idleBlue") : translate("idlePink"), p.x-6, p.y-8);
  }
}
...
document.querySelectorAll(".character-card").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    document.querySelectorAll(".character-card").forEach(b => b.classList.remove("selected"));
    btn.classList.add("selected");
    ui.avatarButton.className = `portrait ${selectedHero}-avatar`;
  });
});
...
ui.restartButton.onclick = resetGame;
ui.avatarButton.ondblclick = activatePower;
ui.languageButton.onclick = () => applyLanguage(currentLang === "en" ? "zh" : "en");
...
addEventListener("keydown", e => {
  ...
  if(!running && ["Space","ArrowUp","ArrowLeft","ArrowRight","KeyA","KeyD","KeyW"].includes(e.code)){
    running = true;
  }
